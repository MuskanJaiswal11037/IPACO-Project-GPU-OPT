 ...
 *************************************************************************/
 ...
 #define WARPS_PER_BLOCK 64     /* one warp / WMMA tile               */
 /* ------------------------------------------------------------------- */
 ...
 /* ------------------------------------------------------------------ */
/*  Kernel-1 : tmp = α · A · B   (warp 0 → tensor cores,               */
/*                                 warp 1 → ordinary FP32 FMAs)       */
/* ------------------------------------------------------------------ */
__global__ void wmma_kernel1_hybrid(int M,int N,int K,float alpha, const __half *A_h,const __half *B_h, float *tmp){
    ...
    if (warp_id == 0){
        ...
        for (int k=0; k<K; k+=16){
            ...
            wmma::load_matrix_sync(Af, A_h + (tm*16)*K + k, K);
            wmma::load_matrix_sync(Bf, B_h + k*N + tn*16, N);
            wmma::mma_sync(acc, Af, Bf, acc);
        }
        for (int i=0;i<acc.num_elements;i++)
            acc.x[i] *= alpha;
        wmma::store_matrix_sync(tmp + (tm*16)*N + tn*16, acc,
        N, wmma::mem_row_major);
        return;
    }

    /* ------------   FP32 PIPE PATH (warp 1)    ------------- */
    /* Each thread computes 8 elements of the same 16×16 tile  */
    float accum[8]={0};
    #pragma unroll
    for (int k=0;k<K;k++){
        float a = __half2float( A_h[(tm*16 + (lane>>2))*K + k] );  // 4 threads / row
        #pragma unroll
        for (int s=0;s<8;s++){// 8 columns per thread
            int col = (lane&3) + 4*s;    // scatter columns
            float b = __half2float( B_h[k*N + tn*16 + col] );
            accum[s] += a * b;
        }
    }
    ...
    for(int s=0;s<8;s++){
        int col = (lane&3) + 4*s;
        tmp[(tm*16 + (lane>>2))*N + tn*16 + col] =
        alpha * accum[s];
    }
}

/* ------------------------------------------------------------------ */
/*  Kernel-2 : D = β·D + tmp · C   (same split)                       */
/* ------------------------------------------------------------------ */
__global__ void wmma_kernel2_hybrid(int M,int N,int K,float beta, const __half *tmp_h,const __half *C_h, float *D){
    ...
    if (warp_id == 0){/* tensor-core */
        ...
        for (int k=0;k<K;k+=16){
            wmma::fragment<wmma::matrix_a,16,16,16,__half,wmma::row_major> Af;
            wmma::fragment<wmma::matrix_b,16,16,16,__half,wmma::row_major> Bf;
            wmma::load_matrix_sync(Af, tmp_h + (tm*16)*K + k, K);
            wmma::load_matrix_sync(Bf,  C_h  + k*N    + tn*16, N);
            wmma::mma_sync(acc, Af, Bf, acc);
        }
        /* β·D + acc */
        wmma::fragment<wmma::accumulator,16,16,16,float> Dold;
        float* dst = D + (tm*16)*N + tn*16;
        wmma::load_matrix_sync(Dold, dst, N, wmma::mem_row_major);
        for(int i=0;i<acc.num_elements;i++)
        acc.x[i] += beta * Dold.x[i];
        wmma::store_matrix_sync(dst, acc, N, wmma::mem_row_major);
    ...
    for (int k=0;k<K;k++)
    {
    float a = __half2float( tmp_h[(tm*16 + (lane>>2))*K + k] );
    #pragma unroll
    for (int s=0;s<8;s++)
    {
    int col = (lane&3) + 4*s;
    float b = __half2float( C_h[k*N + tn*16 + col] );
    accum[s] += a * b;
    }
    }
    /* β·D + α */
    ...
    int row = (lane>>2);
    int col = (lane&3) + 4*s;
    float *dst = D + (tm*16 + row)*N + tn*16 + col;
    float old  = *dst;
    *dst = accum[s] + beta * old;
    }
}
 ...
 void mm2Cuda(int ni,int nj,int nk,int nl, DATA_TYPE alpha, DATA_TYPE beta,
     ...
     wmma_kernel1_hybrid<<<grid1,block>>> (NI, NJ, NK, alpha, dA_h, dB_h, dTmp);
     f32_to_f16 <<< (NI*NJ+threads-1)/threads,threads >>> (dTmp, dTmp_h, NI*NJ);
     wmma_kernel2_hybrid<<<grid2,block>>>(NI, NL, NJ, beta, dTmp_h, dC_h, dD);
